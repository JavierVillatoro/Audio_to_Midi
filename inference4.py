...
# --- UTILITY: DoubleConv (Usada por U-Net y U-Net+BiLSTM) ---
...
# --- UTILITY: Pad and Cat (Usada por U-Net y U-Net+BiLSTM) ---
def _pad_and_cat(x_up, x_skip):
    diff = x_skip.size(2) - x_up.size(2)
    if diff != 0: 
        # Ajuste para asegurar que el padding sea simétrico o asimétrico de forma correcta
...
# --- D. Modelo U-Net + BiLSTM (TU NUEVA ARQUITECTURA) ---
class AudioUNetWithBiLSTMEnd(nn.Module):
    def __init__(self, n_channels, n_classes):
        ...
        # Salida (usa hidden_size * 2 por ser bidireccional)
    ...
    def forward(self, x):
        ...
        x = self.up3(x)
        x = _pad_and_cat(x, x1)
        x_cnn_out = self.conv_up3(x) # (Batch, 64, Time)
        
        # NUEVO: Procesamiento LSTM
        x_lstm_in = x_cnn_out.permute(0, 2, 1) # (Batch, Time, 64)
        self.lstm.flatten_parameters()
        x_lstm_out, _ = self.lstm(x_lstm_in)   # (Batch, Time, hidden_size*2)
        x_final = x_lstm_out.permute(0, 2, 1)  # (Batch, hidden_size*2, Time)
...
def load_audio_cqt(path):
    """Carga audio y calcula el CQT normalizado en dB."""
...
def predictions_to_midi(predictions, output_path, tempo=120):
    """Convierte un array de etiquetas predichas a un archivo MIDI."""
    mid = mido.MidiFile()
...
def visualize_midi_piano_roll(midi_path, sr=SR, hop_length=HOP):
    """Genera y guarda una imagen del piano roll MIDI."""
    ...
    try:
        ...
        if len(active_notes) == 0: 
            print("⚠️ El archivo MIDI no contiene notas activas para visualizar.")
            return
        ...
        plt.close() 
        print(f"Piano Roll guardado en: {MIDI_IMAGE_PATH}")
    except Exception as e: 
        print(f"❌ Error al generar el Piano Roll: {e}")
        pass
...
def select_model_file():
    ...
    if not models:
        ...
        return None
    print("\n--- MODELOS DISPONIBLES (.pth) ---")
    for i, m in enumerate(models):
        print(f"[{i+1}] {m}")
    while True:
        try:
            choice = input("\nElige el número del modelo a usar (o Q para salir): ")
...
def load_architecture(model_path, device):
    ...
    # --- AÑADIDA DETECCIÓN DE TU NUEVA ARQUITECTURA ---
    if "unet_lstm" in filename:
        ...
        print(">> Detectado arquitectura: U-NET Clásico")
        ...
        print("\n⚠️ No se reconoce el nombre del modelo. Intenta adivinar.")
        print("[1] U-Net + BiLSTM / [2] Transformer / [3] U-Net / [4] CRNN")
        opt = input("¿Cuál es?: ")
        if opt == "1": model = AudioUNetWithBiLSTMEnd(n_channels=INPUT_DIM, n_classes=OUTPUT_DIM)
        elif opt == "2": model = AudioTransNet(input_dim=INPUT_DIM, d_model=128, nhead=4, num_layers=3, output_dim=OUTPUT_DIM)
        elif opt == "3": model = AudioUNet(n_channels=INPUT_DIM, n_classes=OUTPUT_DIM)
    ...
    try:
        ...
        print(f"\n❌ Error cargando pesos (RuntimeError). ¿Es la arquitectura correcta?\nDetalle: {e}")
        return None
    except FileNotFoundError:
        print(f"\n❌ Error: Archivo de modelo {model_path} no encontrado.")
        return None
...
def run_inference(audio_path):
    # --- SELECCIÓN DE MODELO Y RUTAS DE SALIDA ---
    ...
    try:
        ...
        print(f"❌ Falta {NORMALIZATION_PARAMS_PATH} (media y desviación estándar del CQT).")
        return
    ...
    if cqt_features is None: 
        print("❌ Error al cargar o procesar el audio.")
        return
    ...
    # --- POST-PROCESAMIENTO (Limpieza Interactiva) ---
    print("\n--- POST-PROCESAMIENTO ---")
...
if __name__ == '__main__':
    ...
    if os.path.exists(INPUT_AUDIO):
        ...
        print("Por favor, ajusta la variable INPUT_AUDIO con la ruta correcta.")