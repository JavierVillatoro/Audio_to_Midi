...
# --- CAMBIO SOLICITADO: 200 EPOCAS Y SIN EARLY STOPPING ---
n_epochs = 200
BEST_VAL_LOSS = float('inf')

# Scheduler: Reduce el LR si la pérdida se estanca (Mantenemos esto porque ayuda a mejorar)
...
# -------------------------------------------------------------------------
# 4. Loop de Entrenamiento (MODIFICADO)
# -------------------------------------------------------------------------
print(f"--- Iniciando entrenamiento en {device} ---")
print(f"Modelo: AudioTransNet | Epochs: {n_epochs} | Early Stopping: DESACTIVADO")
...
for epoch in range(n_epochs):
    ...
    # Barra de progreso para evitar sensación de que se cuelga
    ...
    # Solo guardamos si supera al mejor anterior, pero NO detenemos el entrenamiento.
    if avg_val_loss < BEST_VAL_LOSS:
        print(f"  ✅ RECORD! ({BEST_VAL_LOSS:.4f} -> {avg_val_loss:.4f}). Guardando checkpoint...")
        BEST_VAL_LOSS = avg_val_loss
        torch.save(model.state_dict(), MODEL_PATH)
    else:
        print(f"  (Sin mejora vs record: {BEST_VAL_LOSS:.4f})")
        
    # Hemos eliminado el bloque "else break"

print("\n--- Entrenamiento Finalizado (200 épocas completas) ---")
...
plt.title('Matriz de Confusión - AudioTransNet (200 Epochs)')
plt.colorbar()
